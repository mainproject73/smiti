...
import axios from 'axios'
  ...
  export function PendingTrainer(props) {

    const {user_id } = useParams();


    // Temporary storage of DB data
    const [trainerpend, settrainerpend] = useState({user_id:"",name:"",email:"", address:"", qualification:"" ,specilization:""});
  ...
  async function clicked(){
    let usertype = option
    let id = trainerpend.user_id
    let name=trainerpend.name
    let email=trainerpend.email
    if (usertype== "Administrator" || usertype== "Therapist" || usertype== "Parent" ){
    await fetch(`http://localhost:5000/api/pending/${user_id}/approve`, {
      method: 'post',
      body: JSON.stringify({ user_id,name,email,usertype}),
      
            headers: {
                'Content-Type': 'application/json'
            }
            });
            alert("Thearspist approved")
            navigate(`/approve`)
            }
    else{
      // alert("Select Thearspist Type");
    }
    }
